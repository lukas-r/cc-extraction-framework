...
public class PairTest {
	...
	public static void evaluate(File pairDbFile, File sourceDbFile, File resultsDbFile, TestType testType, PairType pairType, CountAttribute countAttribute, int minSupport, int n, boolean forceRandomPicks) throws SQLException {
		...
		double testTruth;
		if (testType == TestType.PER_PATTERN) {
			List<Pair<String, List<Pair<Integer, PairTestAnswer>>>> results = new ArrayList<Pair<String, List<Pair<Integer, PairTestAnswer>>>>();
			List<Pair<String, List<Pair<Integer, Pair<String, String>>>>> pairs = samplePairsByPattern(sourceDb, pairDb, pairType, countAttribute, minSupport, n, forceRandomPicks);
			
			patternLoop:
			for (int p = 0; p < pairs.size(); p++) {
				Pair<String, List<Pair<Integer, Pair<String, String>>>> pattern = pairs.get(p);
				List<Pair<Integer, PairTestAnswer>> patternResults = new ArrayList<Pair<Integer, PairTestAnswer>>();
				results.add(Pair.of(pattern.getLeft(), patternResults));
				System.out.println("Pattern " + (p + 1) + " of " + pairs.size() + " - \"" + pattern.getLeft() + "\"");
				System.out.println();
				for (int i = 0; i < Math.min(n, pattern.getRight().size()); i++) {
					Pair<Integer, Pair<String, String>> pair = pattern.getRight().get(i);
					PairTestAnswer answer = askPair(i + 1, pair.getRight().getLeft(), pair.getRight().getRight());
					if (answer == null) {
						break patternLoop;
					}
					patternResults.add(Pair.of(pair.getLeft(), answer));
				}
			}
			testTruth = getTrueRate(results.stream()
					.map(p -> p.getRight())
					.flatMap(p -> p.stream())
					.map(r -> r.getRight()));
			...
			for (Pair<String, List<Pair<Integer, PairTestAnswer>>> patternResults: results) {
				double patternTruth = getTrueRate(patternResults.getRight().stream().map(r -> r.getRight()));
				insertPatternResult(testDb, testId, patternResults.getLeft(), patternTruth);
				for (Pair<Integer, PairTestAnswer> answer: patternResults.getRight()) {
					insertPair(testDb, testId, answer.getLeft(), patternResults.getLeft(), answer.getRight().no);
				}
			}
		} else {
	...
	private static PairTestAnswer askPair(int no, String instanceString, String classString) {
		System.out.println(no + ".");
		System.out.println();
		System.out.println(instanceString + "\t\t" + "isA" + "\t\t" + classString);
		for (PairTestAnswer answer: PairTestAnswer.ANSWERS.values()) {
			System.out.println("(" + answer.no + ") " + answer.text);
		}
		System.out.println();
		while (true) {
			...
			String answerString;
			try {
				answerString = new BufferedReader(new InputStreamReader(System.in)).readLine();
			} catch (IOException e) {
				e.printStackTrace();
				continue;
			}
			...
			try {
				...
				if (PairTestAnswer.ANSWERS.keySet().contains(answer)) {
					System.out.println();
					return PairTestAnswer.ANSWERS.get(answer);
				}
